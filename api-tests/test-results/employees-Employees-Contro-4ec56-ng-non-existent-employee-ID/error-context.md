...
- Name: employees.spec.js >> Employees Controller API Tests >> Should return 404 when updating non-existent employee ID
- Location: tests\employees.spec.js:127:3
...
```ts
  ...
  36  |         confirmPassword: testPassword,
  37  |         phoneNumber: '9876543210',
  38  |         role: 'Employee'
  39  |       }
  40  |     });
  41  |     expect(regRes.status()).toBe(200);
  42  | 
  43  |     // 4. Login as new user to get token and retrieve User ID via /me
  44  |     const loginRes = await request.post('/api/auth/login', {
  45  |       data: { email: testEmail, password: testPassword }
  46  |     });
