...
const WhatsAppInboxReview = () => {
    ...
    const fetchMessages = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/WhatsAppConfig/InboxReview?status=${statusFilter}`);
            setMessages(response.data);
        } catch (error) {
            console.error('Error fetching WhatsApp inbox messages:', error);
        } finally {
            setLoading(false);
        }
    };
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '1rem' }}>
                    ...
                    <tbody>
                        {messages.length === 0 ? (
                            ...
                            messages.map(msg => (
                                <tr key={msg.whatsAppInboxMessageId} style={{ borderBottom: '1px solid #eee' }}>
                                    <td>{msg.fromPhone}</td>
                                    <td>{msg.body || '[Media]'}</td>
...
export default WhatsAppInboxReview;
