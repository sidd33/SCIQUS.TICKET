...
function AssignmentReasonPanel({ assignmentReason }) {
  ...
  const colors =
    ...
    ] || ASSIGNMENT_TYPE_COLORS.Queue;

  const hasCandidates =
    (assignmentReason.candidates?.length ?? 0) > 0;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {assignmentReason.assignmentType}
              {assignmentReason.algorithmName
                ? ` · ${assignmentReason.algorithmName}`
                : ''}
            </span>
          ...
          >
            ...
            {hasCandidates && (
              ...
              >
                ...
                >
                  Why this employee over others (
                  {assignmentReason.candidates.length}{' '}
                  eligible)
                  <ChevronDown
                    size={12}
                    style={{
                ...
                {showCandidates && (
                  ...
                  >
                    ...
                    <tbody>
                      {assignmentReason.candidates.map(
                        c => (
                          <tr
                            key={c.employeeId}
                            style={{
                          ...
                          >
                            ...
                            >
                              {c.hoursSinceLastAssignment.toFixed(
                                1
                              )}
                            </td>
                          </tr>
...
export default function TicketDetails() {
  ...
  const [isInternalComment, setIsInternalComment] =
    useState(false);
  ...
  async function loadTicketDetails() {
    ...
    try {
      ...
      try {
        ...
        setHistories(
          hRes.data?.items || hRes.data || []
        );
      } catch (err) {
        console.error(
          'Failed to load ticket timeline:',
          err
        );
      ...
      try {
        const arRes = await api.get(
          `/tickets/${ticketId}/assignment-reason`
        );

        setAssignmentReason(arRes.data);
      } catch (err) {
        console.error(
          'Failed to load assignment reason:',
          err
        );
  ...
  const handleDeleteTicket = async () => {
    const confirmed = window.confirm(
      `Are you sure you want to delete ${
        ticket?.ticketNumber || 'this ticket'
