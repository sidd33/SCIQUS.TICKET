...
export default function SupportPlans() {
  ...
  async function loadPlans() {
    ...
    try {
      ...
      /*
       * Normalize the email notification value coming from the API.
       * This also protects against older records where the property
       * may not exist yet.
       */
      const normalizedPlans = list.map(plan => ({
        ...plan,
        emailNotificationsEnabled:
          plan.emailNotificationsEnabled ??
          plan.EmailNotificationsEnabled ??
          false
      }));
      ...
      if (normalizedPlans.length > 0) {
        setSelectedPlanId(normalizedPlans[0].supportPlanId);
      }
    } catch (err) {
      setError(
        err.response?.data?.message ||
        'Failed to load support plans.'
      );
    } finally {
  ...
  const handleCreatePlan = async (e) => {
    ...
    try {
      await api.post('/SupportPlan', {
        ...
        emailNotificationsEnabled:
          Boolean(newPlan.emailNotificationsEnabled)
      });

      setMessage(
        `Support plan "${newPlan.name}" created successfully.`
      );
  ...
  const handleAssignPlan = async (e) => {
    ...
    if (!selectedAccId || !selectedPlanId) {
      return;
    }
    ...
    try {
      ...
      setMessage(
        'Support plan assigned to customer account successfully with entitlement counters!'
      );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await api.put(
        `/SupportPlan/${selectedPlan.supportPlanId}`,
        {
          name: editForm.name,
          ...
          blockWhenExhausted:
            editForm.blockWhenExhausted,
          emailNotificationsEnabled:
            Boolean(editForm.emailNotificationsEnabled)
        }
      );

      setMessage(
        `Support plan "${editForm.name}" updated successfully.`
      );
  ...
  return (
    <div className="tickets-page">
      ...
      <div className="page-header">
        <div>
          <h1>
            ...
            />{' '}
            Support Plans & Ticket Entitlements
          </h1>

          <p>
            Define account support tiers, ticket quotas, overage
            policies, and account assignments.
          </p>
        </div>
      ...
      {showCreateForm && (
        ...
        >
          ...
          >
            ...
            >
              <input
                id="create-email-notifications"
                type="checkbox"
                checked={
                  Boolean(
                    newPlan.emailNotificationsEnabled
                  )
                }
                onChange={(e) =>
                  setNewPlan({
              ...
              <label
                htmlFor="create-email-notifications"
                style={{
                  color: '#cbd5e1',
                  fontSize: '0.85rem',
                  cursor: 'pointer'
      ...
      >
        ...
        >
          <UserCheck size={18} />
          Assign Support Plan to Customer Account
        </h3>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setEditForm({
                        ...selectedPlan,
                        emailNotificationsEnabled:
                          selectedPlan.emailNotificationsEnabled ??
                          false
                      });
