...
export default function Departments() {
  ...
  // Auto-assign algorithm weights
  const [wLoad, setWLoad] = useState(0.2);
  const [wSeverity, setWSeverity] = useState(0.5);
  const [wRecency, setWRecency] = useState(0.3);
    ...
    try {
      if (editingDept) {
        await api.put(`/departments/${editingDept.departmentId}`, {
          ...
          ticketAutoAssignMethod: assignMethod,
          w_Load: Number(wLoad),
          w_Severity: Number(wSeverity),
          w_Recency: Number(wRecency)
        });

        setMessage({
          type: 'success',
          text: `Department '${deptName}' updated successfully.`
        });
      } else {
        ...
        setMessage({
          type: 'success',
          text: `Department '${deptName}' created successfully.`
        });
      }
      ...
      setMessage({
        type: 'error',
        text: 'Failed to save department details.'
      });
    }
  };
  ...
  const handleCreateEmployee = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: `Employee '${newEmp.name}' created and assigned to ${selectedDept.name}.`
      });
  ...
  const handlePromoteDeptHead = async (empId) => {
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Promoted staff member to Department Head!'
      });
      ...
      setMessage({
        type: 'error',
        text: err.response?.data?.message || 'Failed to promote employee.'
      });
    }
  };
  ...
  const deptEmployees = selectedDept
    ? employees.filter(
        e => e.departmentId === (selectedDept.id || selectedDept.departmentId)
      )
    : [];
    ...
    <div className="tickets-page">
      <div className="page-header">
        <div>
          ...
          <p>
            Inspect department structures, manage staff assignments, and create
            new employees.
          </p>
        </div>
        ...
        <button
          ...
          onClick={() => {
            ...
            setDeptName('');
            setDeptDescription('');
            setAssignMethod('Auto_assignment_custom');
            setWLoad(0.2);
            setWSeverity(0.5);
            setWRecency(0.3);
      ...
      >
        {departments.map(d => {
          const empList = employees.filter(
            e => e.departmentId === (d.id || d.departmentId)
          );

          const deptHead = employees.find(
            e => e.id === d.departmentHeadId
          );
            ...
            >
              <div>
                ...
                >
                  ...
                  >
                    {d.ticketAutoAssignMethod === 'RoundRobin'
                      ? 'Round-Robin'
                      : d.ticketAutoAssignMethod === 'LoadBalanced'
                      ? 'Load-Balanced'
                      : 'Weighted Score'}
                  </span>
                </div>
                ...
                >
                  {d.description ||
                    'Primary organizational operational unit.'}
                </p>
                ...
                {deptHead && (
                  ...
                  >
                    ...
                    <span>
                      Dept Head: <strong>{deptHead.name}</strong>
                    </span>
                  </div>
                )}
              </div>
                ...
                >
                  <button
                    ...
                    onClick={() => {
                      ...
                      setAssignMethod(
                        d.ticketAutoAssignMethod ||
                          'Auto_assignment_custom'
                      );
                      setWLoad(d.w_Load ?? 0.2);
                      setWSeverity(d.w_Severity ?? 0.5);
                      setWRecency(d.w_Recency ?? 0.3);
                      setShowDeptForm(true);
                    }}
                  >
                    <Edit2 size={13} /> Edit
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                <strong style={{ color: 'white' }}>
                  {selectedDept.ticketAutoAssignMethod === 'RoundRobin'
                    ? 'Round-Robin Rotation'
                    : selectedDept.ticketAutoAssignMethod === 'LoadBalanced'
                    ? 'Load-Balanced'
            ...
            {deptEmployees.length === 0 ? (
              ...
              >
                No employees currently assigned to this department. Click
                'Create & Add Employee' above to add staff members.
              </div>
            ) : (
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {deptEmployees.map(emp => {
                      const isHead =
                        emp.id === selectedDept.departmentHeadId;
                      ...
                      return (
                        <tr key={emp.id}>
                          ...
                          <td>
                            {emp.designation || 'Support Specialist'}
                          </td>
            ...
            <form onSubmit={handleCreateEmployee}>
              ...
              <div className="form-group">
                ...
                <input
                  required
                  type="email"
                  placeholder="e.g. robert.vance@sciqustickets.com"
                  value={newEmp.email}
                  onChange={e =>
                    setNewEmp({
                      ...newEmp,
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSaveDept}>
              ...
              <div className="form-group">
                <label>
                  Per-Department Auto-Assign Routing Method
                </label>
                ...
                >
                  <option value="Auto_assignment_custom">
                    Weighted Score (Multi-Factor Formula)
                  </option>
