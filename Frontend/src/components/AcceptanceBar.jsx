...
export default function AcceptanceBar({ ticketId, deadline, onAction }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        {!rejecting ? (
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button className="btn btn--success btn--sm" onClick={handleAccept} disabled={submitting}>
              <CheckCircle2 size={14} /> Accept Ticket
            </button>
            <button className="btn btn--danger btn--sm" onClick={() => setRejecting(true)} disabled={submitting}>
              <XCircle size={14} /> Reject Ticket
            </button>
          </div>
        ) : (
          <form onSubmit={handleReject} style={{ display: 'flex', gap: '0.5rem', width: '100%', marginTop: '0.5rem' }}>
            <input
              placeholder="Enter rejection reason..."
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              required
              style={{ flex: 1, padding: '0.4rem 0.75rem', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: 'white', fontSize: '0.8rem' }}
            />
            <button type="submit" className="btn btn--danger btn--sm" disabled={submitting || !reason.trim()}>
              Confirm Reject
            </button>
