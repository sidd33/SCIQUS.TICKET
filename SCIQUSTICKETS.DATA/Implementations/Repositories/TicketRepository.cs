...
namespace SCIQUSTICKETS.DATA.Implementations.Repositories
{
	public class TicketRepository : GenericRepository<Ticket>, ITicketRepository
	{
			...
			int pageSize)
		{
			...
			if (ticketSubTypeId.HasValue)
				query = query.Where(t => t.TicketSubTypeId == ticketSubTypeId);

			if (priorityId.HasValue)
				query = query.Where(t => t.PriorityId == priorityId);

			if (businessImpactId.HasValue)
				query = query.Where(t => t.BusinessImpactId == businessImpactId);
			...
			if (isInternal.HasValue)
				query = query.Where(t => t.IsInternal == isInternal);

			if (isOpen.HasValue)
				query = query.Where(t => t.IsOpen == isOpen);
	...
	.ThenInclude(c => c.CommentedByUser)
		...
		public async Task<string> GenerateTicketNumberAsync()
		{
			...
			store.LastUpdatedDate = DateTime.UtcNow;


			await _context.SaveChangesAsync();


			return $"TKT-{store.CurrentNumber:D6}";
		}
